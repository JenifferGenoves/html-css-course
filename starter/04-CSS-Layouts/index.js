...
/* function listPaddockManagersByName() {
  paddockManagers.sort((a,b) =>parseInt(a.taxNumber) - parseInt(b.taxNumber));
  return paddockManagers.map((element) => element.taxNumber);
};
 */

// Supuesto: Orden alfabético ascendente
function listPaddockManagersByName() {
return paddockManagers.sort((a,b) => a.name.localeCompare(b.name)).map(e => e.taxNumber)
};
...
return paddockType.map(paddockTypes => {
  let totalArea = 0;
  paddocks.forEach(paddocksItem =>(paddocksItem.paddockTypeId == paddockTypes.id) ? totalArea = totalArea + paddocksItem.area : '')
  return { name: paddockTypes.name, area:totalArea };
  }).sort((a,b) => b.area - a.area).map((paddockTypes) => paddockTypes.name) }
...
function sortFarmManagerByAdminArea() {
  return paddockManagers.map(paddockManager => {
    let totalArea = 0;
    paddocks.forEach(paddocksItem =>(paddocksItem.paddockManagerId == paddockManager.id) ? totalArea = totalArea + paddocksItem.area : '')
    return { name: paddockManager.name, area:totalArea };
    }).sort((a,b) => b.area - a.area).map((paddockManager) => paddockManager.name) }
